...
const TrailingStopRulesPanel = ({ trailingStopRules = [], onChange }) => {
  const rule = trailingStopRules[0] || {};
  const isCustom = rule.secondary_source === "Custom";
  return (
    <div className="border border-gray-300 rounded p-4 w-[220px]">
      <h2 className="text-md font-semibold mb-2 text-gray-700">Trailing Stop Rules</h2>
      <div className="flex flex-col gap-1 text-sm">
        <div className="flex justify-between items-center">
          <span className="text-gray-700">Primary Source</span>
          <select
            value={rule.primary_source ?? ""}
            onChange={e => onChange && onChange("trailing_stop_rules.0.primary_source", e.target.value)}
        ...
        <div className="flex justify-between items-center">
          ...
          <select
            ...
            onChange={e => onChange && onChange("trailing_stop_rules.0.condition", e.target.value)}
            className="border px-2 py-1 rounded w-20 text-right bg-white"
          >
            <option value="">--</option>
            <option value=">=">&gt;=</option>
        ...
        <div className="flex justify-between items-center">
          ...
          <select
            ...
            onChange={e => onChange && onChange("trailing_stop_rules.0.secondary_source", e.target.value)}
            className="border px-2 py-1 rounded w-20 text-right bg-white"
          >
            <option value="">--</option>
            <option value="Price">Price</option>
        ...
        <div className="flex justify-between items-center">
          ...
          {isCustom ? (
            <input
              ...
              className="border px-2 py-1 rounded w-20 text-right bg-white"
              value={rule.value ?? ""}
              onChange={e => onChange && onChange("trailing_stop_rules.0.value", e.target.value)}
            />
          ) : (
            <input
              type="number"
...
export default TrailingStopRulesPanel; 